        ...
        for p in self.generator:
            ...
            if not p.exists():
                if self.getOption('test'):
                    pywikibot.output('SKIPPED:[[%s]]' % p.title())
                continue
            result = self.treat(p)
            if result:
                if self.getOption('test'):
                    pywikibot.output('WD FOUND:[[%s]]' % p.title())
            else:
                pywikibot.output('WD NOT FOUND:[[%s]]' % p.title())
                marked += 1
                wdData.append(result)
                if self.getOption('test'):
                    pywikibot.output('RESULT:[[%s]]' % result)
