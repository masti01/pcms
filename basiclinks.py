...
class BasicBot(
    ...
    def get_archive_memento(self, url):
        #get URI with memento_client
        dt = datetime.datetime.now()
        mc = MementoClient()
        try:
            return(mc.get_memento_info(url, dt).get("mementos").get("closest").get("uri")[0])
        except:
            return(None)

    def get_archive(self, url):
        # get form archive org directly
         try:
             url2 = urllib.request.urlopen('https://web.archive.org/web/' + url).geturl()
             #url2 = urllib.request.urlopen('https://arquivo.pt/wayback/' + url).geturl()

             # FIXME: Hack for T167463: Use https instead of http for archive.org links
             if url2.startswith('http://web.archive.org'):
                 url2 = url2.replace('http://', 'https://', 1)
             #pywikibot.output('archive=%s' % url2)
             return(url2)
         except:
             pywikibot.output('ERROR:%s' % url)
             return(None)
    ...
    def treat(self,page) -> None:
        ...
        for m in linkR.finditer(text):
            if m.group('url'):
                ...
                continue
            #url = 'http://www.masti.error.com'
            if url.startswith('http://sports-reference.com/olympics'):
                result['url'] = url
                result['arch'] = self.get_archive(url)
                result['memento'] = self.get_archive_url(url)
                result['client'] = self.get_archive_memento(url)
    ...
    def generateresultspage(self, redirlist, pagename, header, footer):
        ...
        for p in redirlist.keys():
            if self.getOption('test'):
                pywikibot.output(p)
            finalpage += u'\n|-\n| [[%s]] || %s || %s || %s || %s' % (p, redirlist[p]['url'], redirlist[p]['arch'], redirlist[p]['memento'], redirlist[p]['client'])
    ...
    def run(self):
        ...
        header += '! strona !! link !! archive.org !! memento !! mementoClient'
