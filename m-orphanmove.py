...
class BasicBot(
    ...
    def run(self):
        ...
        for page in self.generator:
            ...
            counter += 1
            self.moveOrphan(page)
            marked += 1

    def moveOrphan(self, page):
        '''
        ...
        if self.getOption('test'):
            pywikibot.output(u'Template:%s' % orphan)
            pywikibot.input('Waiting...')
        page.text = re.sub(orphanR, u'', text)
        if self.getOption('test'):
            pywikibot.output(u'Page:%s' % page.text)
            pywikibot.input('Waiting...')
        page.save(self.getOption('summary'))

        talkPage = page.toggleTalkPage()
        if not u'{{Sierotka' in page.text:
            talkPage.text = orphan + talkPage.text
            if self.getOption('test'):
